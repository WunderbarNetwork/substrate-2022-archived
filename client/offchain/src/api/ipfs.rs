...
impl IpfsApi {
	...
	pub fn request_start(&mut self, request: IpfsRequest) -> Result<IpfsRequestId, ()> {
		...
		match self.next_id.0.checked_add(1) {
			...
			None => {
				tracing::error!("Overflow in offchain worker IPFS request ID assignment");
				return Err(())
			},
		};
		...
		loop {
			...
			match next_message {
				Some(WorkerToApi::Response { id, value }) => match self.requests.remove(&id) {
					...
					_ => tracing::error!("State mismatch between the API and worker"),
				},
				...
				None => {
					tracing::error!("Worker has crashed");
					return ids
						.iter()
						.map(|_| {
